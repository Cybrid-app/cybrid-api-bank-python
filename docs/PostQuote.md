...
**destination_account_guid** | **str, none_type** | The unique identifier for the destination account. Used to determine routing-specific fees for EFT withdrawals. Required when product_type is inter_account. Optional when product_type is funding. | [optional] 
**payment_rail** | **str, none_type** | The payment rail the resulting transfer will execute on. Required when the downstream transfer will be an e-transfer withdrawal. Optional when product_type is funding. | [optional] 
...
**destination_accounts** | [**[PostQuoteEntry]**](PostQuoteEntry.md) | Destination accounts for batch transactions on UTXO-based blockchains. A single destination account is required for Base blockchain assets. Optional when product_type is crypto_transfer. | [optional] 
